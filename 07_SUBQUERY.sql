...
(SELECT AVG(SALARY) 
FROM EMPLOYEE)
ORDER BY JOB_CODE; -- 그룹함수를 사용했기에, 단일행 -> 1개임


-- 가장 적은 급여를 받는 직원의
-- 사번, 이름, 직급, 부서코드, 급여, 입사일을 조회

SELECT EMP_ID , EMP_NAME , JOB_NAME, DEPT_CODE , SALARY ,ENT_DATE 
FROM EMPLOYEE 
JOIN JOB USING (JOB_CODE)
WHERE SALARY 
...
SELECT MAX(SALARY)
FROM EMPLOYEE
GROUP BY DEPT_CODE; -- 7행 1열 다중행 단일열
...
-- 정렬을 하려고 할 때, RESULT SET에 대한 SELECT 구문이 또 필요하다.....
...
-- > ANY : 과장 급여 3개중에서 최소 금액인 220 보다 더 큰 SALARY 조회
-- < ANY : 과장 급여 3개중에서 최대 금액인 376 보다 더 작은 SALARY 조회
...
JOIN JOB USING(JOB_CODE)
WHERE JOB_NAME = '과장'
AND SALARY > ALL (SELECT SALARY /* MAX(SALARY) 써도 된다. */
FROM EMPLOYEE 
NATURAL JOIN JOB
WHERE JOB_NAME = '차장');
...
--WHERE ENT_YN = 'Y'
--AND SUBSTR(EMP_NO,8,1) = '2');
-- ORA-00913: 값의 수가 너무 많습니다 --> 비교하려는 행의 개수가 다르기 때문에 에러
...
FROM EMPLOYEE
ORDER BY SALARY DESC);
-- 전형돈, 윤은해가 월급이 똑같은데 서로 순위가 19위 20위임


-- 2) RANK() OVER(정렬순서) 로 해결하기

SELECT RANK() OVER(ORDER BY SALARY DESC) 순위, EMP_NAME, SALARY
FROM EMPLOYEE;

-- 전형돈과 윤은해가 같이 19위가 되었고, 20위가 사라지고 21위부터 시작하게 됨


-- DENSE_RANK() OVER : 동일한 순위 이후의 등수를 이후의 순위로 계산
-- 공동 1위가 두명이어도 다음 순위는 2위임
...
-- 전형돈과 윤은해가 같이 19위가 되었고, 다음 순위는 20위 부터 시작하게 됨

-- RANK() OVER(), DENSE_RANK() OVER() 시험에 나옴
...
-- 9. WITH
-- 서브쿼리에 이름을 붙혀주고, 사용 시 이름을 사용하게 함.
